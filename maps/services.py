...
def get_origin_states(data_dict):
    ...
    if(origin_type == 'origin_system'):
        return set(GraphNode.objects.filter(system__name=data_dict['origin_system']).values_list('id',flat=True))
    
    elif(origin_type == 'origin_station'):
        return {Station.objects.values_list('id',flat=True).get(name=data_dict['origin_station'])}
...
def get_destination_states(data_dict):
    ...
    if(destination_type == 'destination_system'):
        return set(GraphNode.objects.filter(system__name=data_dict['destination_system']).values_list('id',flat=True))
    
    elif(destination_type == 'destination_station'):
        return {Station.objects.values_list('id',flat=True).get(name=data_dict['destination_station'])}
        
    elif(destination_type == 'destination_region'):
        return set(GraphNode.objects.filter(system__region__name=data_dict['destination_region']).values_list('id',flat=True))
...
def get_avoided_systems(data_dict):
    ...
    return avoided_systems | set(MapSolarSystem.objects.filter(region_id__in=avoided_regions).values_list('id', flat=True))
...
def compute_travel_path(data_dict):
    ...
    if(data_dict['use_midpoints'] and len(data_dict['waypoint_list']) > 0):
        ...
        for destination in data_dict['waypoint_list']:
            ...
            waypoint_list, waypoint_time, waypoint_error = compute_waypoint_path(origin_states, destination_states,  data_dict)
...
def compute_waypoint_path(origin_states, destination_states, data_dict):
    ...
    duration_func = get_travel_duration_func(align_time, ship_speed, warp_speed, data_dict['autopilot'])
...
def compute_travel_error(data_dict, path):
    ...
    if(data_dict['autopilot']):
        ...
        min_approach = travel_time.compute_approach_time(11000, data_dict['ship_speed'], data_dict['align_time'])
        max_approach = travel_time.compute_approach_time(14000, data_dict['ship_speed'], data_dict['align_time'])
...
def get_travel_duration_func(align_time, ship_speed, warp_speed, is_autopilot):
    ...
    if(is_autopilot):
        times = {
            ...
            'wait_approach': 3,
            'approach': travel_time.compute_approach_time(12500, ship_speed, align_time),
            }
    else:
        times = {
            'wait_begin': 4,
    ...
    def compute_travel_duration(distance):
        return non_warp_time + travel_time.compute_warp_time(distance, ship_speed, warp_speed)
