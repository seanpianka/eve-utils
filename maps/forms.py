...
from static_dump.forms import SystemNameField, RegionNameField, MultiSystemNameField, MultiRegionNameField, StationNameField
...
class ConditionalForm(forms.Form):
	def validate_required_field(self, cleaned_data, field_name, message="This field is required"):
		...
		if((field_name not in self._errors) and (cleaned_data.get(field_name, None) is None)):
...
class PathForm(ConditionalForm):
	origin_type = 					forms.ChoiceField(widget=forms.RadioSelect, choices=origin_type_choices, initial="origin_system",label="Origin")
	
	origin_system = 				SystemNameField(required=False)
	origin_station = 				StationNameField(required=False)
	
	destination_type = 				forms.ChoiceField(widget=forms.RadioSelect, choices=destination_type_choices, initial="destination_system",label="Destination")
	...
	maximum_security = 				forms.ChoiceField(choices=security_choices, label="Avoid Security >")
		...
		if('destination_type' in cleaned_data):
			...
			if(dest_type == 'destination_waypoints'):
				if(len(self.waypoint_list) == 0):
					self._errors['destination_type'] = self.error_class(["At least one waypoint is required."])
...
class WaypointForm(ConditionalForm):
	destination_type = 			forms.ChoiceField(widget=forms.RadioSelect, choices=waypoint_type_choices, initial="destination_system",label="Destination")
