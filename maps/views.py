...
def shortest_path(request):
    ...
    if(len(request.GET) > 0):
        ...
        if(path_form.is_valid()):
            ...
            path, path_length, path_time, path_error = compute_travel_path(path_form.cleaned_data)
    ...
    destination_types = zip(
                ...
                [{'field':path_form['destination_system']}, {'field':path_form['destination_station']}, {'field':path_form['destination_region']}]
                )

    return render_to_response(
        'maps/shortest_path.html',
        {'path_form':path_form, 'destination_types':destination_types, 'origin_types':origin_types, 'waypoint_list':waypoint_list,
            'path':path, 'path_length':path_length, 'path_time':path_time, 'path_error':path_error},
...
def manage_waypoints(request):
    ...
    destination_types = zip(
                ...
                [waypoint_form['destination_system'], waypoint_form['destination_station'], waypoint_form['destination_region'], ]
                )
    ...
    waypoint_form_str = render_to_string('include/waypoint_form.html', {'form':waypoint_form, 'destination_types':destination_types}, context_instance=context_inst)
...
def add_waypoint(request):
    ...
    if(waypoint_form.is_valid()):
        ...
        if(response_data['type'] == 'destination_system'):
            ...
            system = MapSolarSystem.objects.values('id','name','security_level').get(name=waypoint_form.cleaned_data['destination_system'])
            ...
            station = Station.objects.values('id','name','solar_system__security_level').get(name=waypoint_form.cleaned_data['destination_station'])
            ...
            region = MapRegion.objects.values('id','name').get(name=waypoint_form.cleaned_data['destination_region'])
        ...
        context_inst = RequestContext(request)
        response_text = render_to_string('include/waypoint_entry.html', {'point': response_data, 'num':-1}, context_instance=context_inst)
        ...
        context_inst = RequestContext(request)
        response_text = render_to_string('include/waypoint_form.html', {'form':waypoint_form, 'destination_types':destination_types}, context_instance=context_inst)
...
def save_waypoints(request):
    ...
    if(request.method == 'POST' and 'waypoints' in request.POST):
        ...
        try:
            ...
            if(len(system_dict) < len(system_set) or len(system_dict) < len(system_set) or len(system_dict) < len(system_set)):
                raise WaypointValidationException()
            ...
            for item in waypoint_list:
                ...
                if(item['type'] == 'destination_region'):
                    ...
                    saved_waypoints.append({'type':'destination_region', 'id':region.id, 'name':region.name, 'security':None})
                    
                elif(item['type'] == 'destination_system'):
                    system = system_dict[object_id]
                    saved_waypoints.append({'type':'destination_system', 'id':system.id, 'name':system.name, 'security':system.security_level})
                    
                else:
                    station = station_dict[object_id]
                    saved_waypoints.append({'type':'destination_station', 'id':station.id, 'name':station.name, 'security':station.solar_system.security_level})
